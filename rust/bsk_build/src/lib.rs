...
//! ``bsk_messages::BskModuleRuntime``. It mirrors the ``SysModel`` fields
...
//! # `bsk_module!()`
//!
//! [`bsk_module!()`] includes the shim this crate generates. It lives here
//! (not in ``bsk-messages``) because it's this crate's own output being
//! included — add ``bsk-build`` as a normal dependency too (default
//! features off; the macro itself needs neither `syn` nor `walkdir`),
//! alongside the existing ``[build-dependencies]`` entry:
//!
//! ```toml
//! [dependencies]
//! bsk-build = { path = "...", default-features = false }
//!
//! [build-dependencies]
//! bsk-build = { path = "..." }
//! ```
...
fn render_c_header(info: &ConfigInfo, module: &str) -> String {
    ...
    // Runs the struct's normal Rust drop glue (frees any `Option<Box<T>>`
    // owned-state field) whenever the C++ side destroys it — see
    // "Owned heap state" in the module docs and `BSK_RUST_DECL`'s
    // `Drop_<module>` declaration, defined out-of-line just below.
    // Only visible to C++ (SWIG); plain-C consumers see an ordinary struct.
    h.push_str(
        "#ifdef __cplusplus\n\
         \x20   ~",
    );
...
fn render_shim(info: &ConfigInfo, module: &str) -> String {
    ...
    s.push_str(cfg_type);
    s.push_str(
        " as BskModule>::self_init(&mut *cfg);\n\
         }\n\n",
    );
...
mod tests {
    ...
    #[test]
    fn header_declares_and_defines_cpp_destructor() {
        let info = info_for(
            "#[repr(C)] pub struct MyConfig { \
             pub runtime: bsk_messages::BskModuleRuntime, \
             pub state: Option<Box<MyState>>, \
        ...
        // Struct must have a tag name (not anonymous) for `~MyConfig()` to
        // be a legal in-class destructor declaration under C++.
        ...
        // The BSK_RUST_DECL macro (bsk_rust_module.h) declares Drop_<name>
        // alongside SelfInit/Reset/Update — not re-declared here.
    ...
    #[test]
    fn shim_generates_drop_function_using_drop_in_place() {
        let info = info_for(
            "#[repr(C)] pub struct MyConfig { \
             pub runtime: bsk_messages::BskModuleRuntime, \
             pub state: Option<Box<MyState>>, \
